...
class Client {
  ...
  Client(
    ...
    assert(endPoint.startsWith(RegExp("http://|https://")),
  ...
  Future<Directory> _getCookiePath() async {
    final directory =
        await PathProvider.instance.getApplicationDocumentsDirectory();
  ...
  Future<Response> call(HttpMethod method,
    ...
    try {
      ...
      if (responseType == ResponseType.bytes) {
        if ((e.response!.headers.value('content-type') ?? '')
      ...
      if ((e.response!.headers.value('content-type') ?? '')
          .contains('application/json')) {
        throw AppwriteException(e.response!.data['message'],
            e.response!.data['code'], e.response!.data);
      } else {
        throw AppwriteException(
            e.response!.data, e.response!.statusCode, e.response!.data);
      }
    } catch (e) {
