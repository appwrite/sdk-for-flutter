...
class Oauth2 extends Service {
  ...
  Future<models.Oauth2Approve> approve({required String projectId, required String grantId, String? authorizationDetails}) async {
    final String apiPath = '/oauth2/{project_id}/approve'.replaceAll('{project_id}', projectId)+ '?project=${Uri.encodeComponent(client.config['project'] ?? '')}';
  ...
  /// redirect.
  Future<models.Oauth2Authorize> authorize({required String projectId, required String clientId, required String redirectUri, required String responseType, required String scope, String? state, String? nonce, String? codeChallenge, String? codeChallengeMethod, String? prompt, int? maxAge, String? authorizationDetails}) async {
    final String apiPath = '/oauth2/{project_id}/authorize'.replaceAll('{project_id}', projectId)+ '?project=${Uri.encodeComponent(client.config['project'] ?? '')}';
  ...
  Future<models.Oauth2Grant> createGrant({required String projectId, required String userCode}) async {
    final String apiPath = '/oauth2/{project_id}/grants'.replaceAll('{project_id}', projectId);
  ...
  Future<models.Oauth2Grant> getGrant({required String projectId, required String grantId}) async {
    final String apiPath = '/oauth2/{project_id}/grants/{grant_id}'.replaceAll('{project_id}', projectId).replaceAll('{grant_id}', grantId);
  ...
  Future<models.Oauth2Reject> reject({required String projectId, required String grantId}) async {
    final String apiPath = '/oauth2/{project_id}/reject'.replaceAll('{project_id}', projectId)+ '?project=${Uri.encodeComponent(client.config['project'] ?? '')}';
